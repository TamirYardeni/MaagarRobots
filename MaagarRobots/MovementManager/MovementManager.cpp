...
MovementManager::MovementManager(HamsterAPI::Hamster * hamster, RobotManager * robotManager, VisualDisplay * visualDisplay)
{
...
void MovementManager::NavigateToWaypoint(Location * waypoint)
{
	...
	while (!isWaypointReached)
	{
		...
		if (currDeltaYaw > YAW_TOLERANCE)
		{
			...
			{
				cout << "Stack! going backwards" << endl;
		...
		else
		{
			...
			cout << "finish calculating angle(yaw)! continue move" << endl;
			MoveToWaypoint();
		}

		RecalculateDistanceFromWaypoint();
		visualDisplay->PrintRouteCvMat();
...
void MovementManager::PrintBeforeTurning()
{
	cout << "Preparing to turn..." << endl <<
		...
		", y = " << currLocation.y <<
		", yaw = " << currYaw << endl <<
 		"current dest waypoint: " << endl <<
		"x = " << waypoint->x <<
		", y = " << waypoint->y << endl <<
		"dest yaw: " << destYaw << endl;
}

void MovementManager::PrintAfterTurning()
{
	cout << "Turning " << chosenDirectionName << " to: " <<
		"x - " << currLocation.x <<
		" y - " << currLocation.y <<
		" yaw - " << currYaw <<
		" the deltaYaw is - " << currDeltaYaw <<
		" turningSpeed: " << turnSpeed << endl;
}

void MovementManager::PrintAfterMoving()
{
	cout << "Moving " << chosenDirectionName << " to: " <<
		"x - " << currLocation.x <<
		"  y - " << currLocation.y <<
		"  yaw - " << currYaw <<
		"  the far for waypoint is: " << distanceFromWaypoint <<
		" and the speed : " << moveSpeed << ")" << endl;
}

void MovementManager::PrintAfterWaypointIsReached()
{
	cout << endl <<
		"Yay! the robot has pass waypoint: x - " << waypoint->x << " y - " << waypoint->y << endl <<
